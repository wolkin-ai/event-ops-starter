...
| Agent-visible observability               | Agents work better when they can inspect logs, traces, and runtime behavior directly.       | Structured route logs and request IDs exist, and `./bin/worktree-harness inspect/logs` now exposes worktree-local process snapshots and recent logs. There is still no local trace view or metrics harness.                                                                                                         | Partial | Add local traces and metrics only when the product complexity justifies them.                  |
...
6. Add worktree-local traces/metrics and background review automation only when the team actually runs multiple agents in parallel.

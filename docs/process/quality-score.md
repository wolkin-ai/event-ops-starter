...
| Local-first startup      | 3     | Docker Compose PostgreSQL and Auth.js demo role login keep setup repo-local while matching hosted deployment boundaries more closely.                                                                                                          |
...
| Local observability      | 3     | Structured logs and request IDs exist, route helpers now write worktree-local trace and metric samples, and `./bin/worktree-harness inspect/logs/observe` exposes process state plus local observability output without hosted infrastructure. |
...
The next maturity jump comes from controlled real review execution.

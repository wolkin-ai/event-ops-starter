...
2. architecture / security / performance の分け方を読み返して整理する
3. `npm run review:suite` を実行して結果を確認する
4. specialist review の個別入口も開いて眺める
5. threshold を「止める線」として頭に置く
...
- architecture / security / performance の違いを一言ずつ言い換えてみる
- threshold を一言で言い換えてみる
- 「チェック」と「review」の違いを一言で言い換えてみる

...
1. 同じ場所を同時に触るとぶつかる、という前提を押さえる
2. worktree を「別の机」として捉える
3. `./bin/worktree-harness create ...` を実行して結果を確認する
4. `inspect` で branch / path / url を確認する
5. task manifest の項目を開いて読む
...
- worktree を一言で言い換えてみる
- task manifest が何を助けるかを一言で言い換えてみる
- `owned paths` と `do not touch` の意味を言葉にしてみる
